...
if(endsWith(table_code, "b25014")) {
  ...
  names(x) <- gsub("006e", "_raw_owner_1", names(x))
  names(x) <- gsub("006m", "_raw_owner_moe_1", names(x))

  names(x) <- gsub("007e", "_raw_owner_2", names(x))
  names(x) <- gsub("007m", "_raw_owner_moe_2", names(x))

  names(x) <- gsub("012e", "_raw_renter_1", names(x))
  names(x) <- gsub("012m", "_raw_renter_moe_1", names(x))

  names(x) <- gsub("013e", "_raw_renter_2", names(x))
  names(x) <- gsub("013m", "_raw_renter_moe_2", names(x))

  # For Overcrowded housing need to aggregate the owner and renter raw values
  x$total_raw <- x$total_raw_owner_1 + x$total_raw_owner_2 +x$total_raw_renter_1 + x$total_raw_renter_2
  x$asian_raw <- x$asian_raw_owner_1 + x$asian_raw_owner_2 +x$asian_raw_renter_1 + x$asian_raw_renter_2
  x$black_raw <- x$black_raw_owner_1 + x$black_raw_owner_2 +x$black_raw_renter_1 + x$black_raw_renter_2
  x$nh_white_raw <- x$nh_white_raw_owner_1 + x$nh_white_raw_owner_2 +x$nh_white_raw_renter_1 + x$nh_white_raw_renter_2
  x$latino_raw <- x$latino_raw_owner_1 + x$latino_raw_owner_2 +x$latino_raw_renter_1 + x$latino_raw_renter_2
  x$other_raw <- x$other_raw_owner_1 + x$other_raw_owner_2 +x$other_raw_renter_1 + x$other_raw_renter_2
  x$pacisl_raw <- x$pacisl_raw_owner_1 + x$pacisl_raw_owner_2 +x$pacisl_raw_renter_1 + x$pacisl_raw_renter_2
  x$twoormor_raw <- x$twoormor_raw_owner_1 + x$twoormor_raw_owner_2 +x$twoormor_raw_renter_1 + x$twoormor_raw_renter_2
  x$aian_raw <- x$aian_raw_owner_1 + x$aian_raw_owner_2 +x$aian_raw_renter_1 + x$aian_raw_renter_2

  # Because we've aggregated these values, will need to calculate a new moe
  # Formula: sqrt(moe1^2 + moe2^2 + moe3^2 + moe4^2)
  x$total_raw_moe <- sqrt(total_raw_owner_moe_1^2 + total_raw_owner_moe_2^2 + total_raw_renter_moe_1^2 + total_raw_renter_moe_2^2)
  x$asian_raw_moe <- sqrt(asian_raw_owner_moe_1^2 + asian_raw_owner_moe_2^2 + asian_raw_renter_moe_1^2 + asian_raw_renter_moe_2^2)
  x$black_raw_moe <- sqrt(black_raw_owner_moe_1^2 + black_raw_owner_moe_2^2 + black_raw_renter_moe_1^2 + black_raw_renter_moe_2^2)
  x$nh_white_raw_moe <- sqrt(nh_white_raw_owner_moe_1^2 + nh_white_raw_owner_moe_2^2 + nh_white_raw_renter_moe_1^2 + nh_white_raw_renter_moe_2^2)
  x$latino_raw_moe <- sqrt(latino_raw_owner_moe_1^2 + latino_raw_owner_moe_2^2 + latino_raw_renter_moe_1^2 + latino_raw_renter_moe_2^2)
  x$other_raw_moe <- sqrt(other_raw_owner_moe_1^2 + other_raw_owner_moe_2^2 + other_raw_renter_moe_1^2 + other_raw_renter_moe_2^2)
  x$pacisl_raw_moe <- sqrt(pacisl_raw_owner_moe_1^2 + pacisl_raw_owner_moe_2^2 + pacisl_raw_renter_moe_1^2 + pacisl_raw_renter_moe_2^2)
  x$twoormor_raw_moe <- sqrt(twoormor_raw_owner_moe_1^2 + twoormor_raw_owner_moe_2^2 + twoormor_raw_renter_moe_1^2 + twoormor_raw_renter_moe_2^2)
  x$aian_raw_moe <- sqrt(aian_raw_owner_moe_1^2 + aian_raw_owner_moe_2^2 + aian_raw_renter_moe_1^2 + total_raw_renter_moe_2^2)
}
...
if (startsWith(table_code, "s2301")) {
  ...
  x$total_raw <- ifelse(df$total_pop <= 0, df$total_pop*df$total_rate/100, NA)
  x$asian_raw <- ifelse(df$asian_pop <= 0, df$asian_pop*df$asian_rate/100, NA)
  x$black_raw <- ifelse(df$black_pop <= 0, df$black_pop*df$black_rate/100, NA)
  x$nh_white_raw <- ifelse(df$nh_white_pop <= 0, df$nh_white_pop*df$nh_white_rate/100, NA)
  x$latino_raw <- ifelse(df$latino_pop <= 0, df$latino_pop*df$latino_rate/100, NA)
  x$other_raw <- ifelse(df$other_pop <= 0, df$other_pop*df$other_rate/100, NA)
  x$pacisl_raw <- ifelse(df$pacisl_pop <= 0, df$pacisl_pop*df$pacisl_rate/100, NA)
  x$twoormor_raw <- ifelse(df$twoormor_pop <= 0, df$twoormor_pop*df$twoormor_rate/100, NA)
  x$aian_raw <- ifelse(df$aian_pop <= 0, df$aian_pop*df$aian_rate/100, NA)
