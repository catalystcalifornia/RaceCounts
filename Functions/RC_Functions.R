...
calc_ranks <- function(x) {
  ...
  x$performance_z_quartile <- cut(x$performance_z, 
                                  breaks=quantile(x$performance_z, probs=seq(0,1, by=0.25), na.rm=TRUE), 
                                  include.lowest=TRUE,
                                  labels=c("lowest", "low", "high", "highest"))
  x$disparity_z_quartile <- cut(x$disparity_z, 
                                breaks=quantile(x$disparity_z, probs=seq(0,1, by=0.25), na.rm=TRUE), 
                                include.lowest=TRUE,
                                labels=c("lowest", "low", "high", "highest"))
...
#proposed_to_postgres <- function(x, table_name, indicator, source) {
proposed_to_postgres <- function(x) {

  #x <- as.data.frame(x)
  #geolevel <- x$geolevel[1] # will need a workaround for DP05 table since geolevel has multiple values

  #drop geolevel column here if necessary

  # make character vector for field types in postgresql db
  charvect = rep('numeric', dim(x)[2])
  # change data type for first three columns
  charvect[1:3] <- "varchar" # first three are characters for geoid, name, geolevel
  # add names to the character vector
  names(charvect) <- colnames(x)

  #### Postgres Connection Setup - can uncomment below to run code without error but probably best to remove/put in it's own function ####
  # # drv
  # drv <- dbDriver("PostgreSQL")
  #
  # # con
  # con <- dbConnect(drv, dbname = "racecounts",
  #                  host = "aws-postgres-db.c5udgz7ro8hq.us-west-2.rds.amazonaws.com", port = 5432,
  #                  user = "postgres", password = "password")

  # dbWriteTable
  dbWriteTable(con, c(rc_schema, table_name), x, overwrite = FALSE, row.names = FALSE)

  # Create comment
  id_col = names(df[1])

  # comment <- paste0("COMMENT ON TABLE ", rc_schema, ".", table_name,  " IS '", indicator, " from ", source, ".'; COMMENT ON COLUMN", rc_schema, ".", table_name, ".county_id IS 'County fips';")

  comment <- sprintf("COMMENT ON TABLE ", rc_schema, ".%s IS '%s from %s.'; COMMENT ON COLUMN ", rc_schema, ".%s.%s IS '%s fips';", table_name, indicator, source, table_name, id_col, geolevel)

  # Tested line 354 with ACS Script, returned this:
  ## "COMMENT ON TABLE v4.arei_race_county_2022 IS 'County and State population by race/ethnicity for RC Place page from ACS 2016-2020, Table DP05. All AIAN, All NHPI, All Latinx, all other groups are one race alone and non-Latinx..'; COMMENT ON COLUMN v4.arei_race_county_2022.geoid IS 'county fips';"
