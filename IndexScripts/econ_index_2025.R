...
rc_schema <- 'v7'
rc_schema_alt <- 'v6'
source <- "American Community Survey (ACS) PUMS 2019-2023, American Community Survey (ACS) 2019-2023 Tables S2301 / S2802 / B19301B-I, and United Ways of California 2023"
...
c_1 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_connected_youth_county_", rc_yr))
c_2 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_employment_county_", rc_yr))
c_3 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_internet_county_", rc_yr))
c_4 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_officials_county_", rc_yr))
c_5 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_per_capita_income_county_", rc_yr))
c_6 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_real_cost_measure_county_", rc_yr))
c_7 <- st_read(conn, query = paste0("SELECT * FROM ", rc_schema, ".arei_econ_living_wage_county_", rc_yr))
...
region_urban_type <- st_read(conn, query = paste0("select county_id, region, urban_type from ", rc_schema_alt, ".arei_county_region_urban_type")) # get region, urban_type
...
index <- paste0("Created ", Sys.Date(), "from ", qa_filepath, ". Includes all issue indicators. Issue area z-scores are the average z-scores for performance and disparity across all issue indicators. This data is")
