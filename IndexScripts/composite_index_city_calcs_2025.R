...
df_education_district <- education_tables %>% left_join(crosswalk, by = "dist_id", relationship = 'many-to-many') %>% filter(!is.na(city_id))
...
city_tables_df <- city_tables_updated %>% arrange(city_id) %>%
  ...
  left_join(education_tables_agg) %>%
...
city_tables_df <- city_tables_df %>% filter(!grepl('University', city_name)) %>% filter(!is.na(city_name))
...
# Apply pop screen depending on index_type selected by user. For SCREENED Index: Drop cities with pop below pop_threshold.
all_index <- all_index %>% filter((index_type == "api_" & total_pop > 10000) | (index_type != "api_"))
...
city_index <- calculate_city_index(all_index, issue_area_threshold, indicator_threshold) %>%
  filter(!is.na(disparity_z | performance_z))

# add county, region data
region_pop <- dbGetQuery(con, paste0("SELECT  city_id, city_name, county_id, county_name, region FROM ", rc_schema, ".arei_city_county_district_table")) %>% unique()
city_index <- city_index %>% select(-city_name) %>% left_join(region_pop, by = 'city_id')
city_index <- unique(city_index) %>%
  select(city_id, city_name, region, total_pop, all_indicators_disp_count, all_indicators_perf_count, issue_disp_count, issue_perf_count,
         ...
         housing_disp_z, housing_perf_z, county_id, county_name, disparity_z_quartile, performance_z_quartile) %>% arrange(-disparity_z)

# clean city_name column - just in case names are not clean already
