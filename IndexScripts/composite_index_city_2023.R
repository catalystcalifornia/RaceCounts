...
## Pull in city (or district) level education indicators
education_list <- filter(rc_list, grepl("_district_2023",table)) # pull only district level indicator tables
education_list <- filter(education_list, grepl("api_",table)) # pull only "api_" (screened) indicator tables
education_list <- education_list [order(education_list$table), ] # alphabetize list of state tables, changes df to list the needed format for next step
...
city_list <- filter(city_list, grepl("api_",table)) # pull only screened indicator tables
city_list <- filter(city_list, !grepl("index", table)) # filter out index in case there is a prev version in postgres
...
city_index_screen <- calculate_city_index(all_index, issue_area_threshold, indicator_threshold) %>% filter(!is.na(city_name))
city_index_screen <- city_index_screen %>% mutate(disparity_rank = dense_rank(-disparity_z), performance_rank = dense_rank(-performance_z))  %>%
  select(city_id, city_name, all_indicators_disp_count, all_indicators_perf_count, disp_values_count, perf_values_count,
         disparity_rank, performance_rank, disparity_z, performance_z, everything()) %>% arrange(-disparity_z)
