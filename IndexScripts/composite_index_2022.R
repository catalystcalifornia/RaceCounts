#### Composite Index Index (z-score) for RC v4 ####
...
source("W:/Project/RACE COUNTS/2022_v4/RaceCounts/RC_Index_Functions.R")
...
c_index_v4 <- full_join(c_index_v4, c_7)
...
c_index_v4 <- calculate_index_z(c_index_v4)
...
index_table <- c_index_v4 %>% select(county_id, county_name, region, urban_type, ends_with("_rank"), quadrant, disparity_z, performance_z, disp_avg, perf_avg, disp_values_count, perf_values_count, ends_with("_disparity_z"), ends_with("_performance_z"), everything())
index_table <- index_table[order(index_table[[5]]), ]  # order by disparity rank
View(index_table)

# Send table to postgres 
index_table_name <- "arei_composite_index_2022"
index <- "QA doc: W:\\Project\\RACE COUNTS\\2022_v4\\Composite Index\\Documentation\\QA_Sheet_Composite_Index.docx Includes all Issue area indexes. Composite index z-scores are the average z-scores for performance and disparity across all issue indexes. This data is"
