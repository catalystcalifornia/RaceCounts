...
setwd("W:/Project/RACE COUNTS/2024_v6/RC_Github/SS/RaceCounts/Visualizations")
...
rc_theme <- hc_theme(
    # race counts colors
  colors = c("purple", "orange", "red", "yellow"),
  chart = list(
...
c_1 <- st_read(con, query = "SELECT * FROM v6.arei_educ_chronic_absenteeism_county_2024")
c_2 <- st_read(con, query = "SELECT * FROM v6.arei_educ_hs_grad_county_2024")
c_3 <- st_read(con, query = "SELECT * FROM v6.arei_educ_gr3_ela_scores_county_2024")
c_4 <- st_read(con, query = "SELECT * FROM v6.arei_educ_gr3_math_scores_county_2024")
c_5 <- st_read(con, query = "SELECT * FROM v6.arei_educ_suspension_county_2024")
c_6 <- st_read(con, query = "SELECT * FROM v6.arei_educ_ece_access_county_2024")
c_7 <- st_read(con, query = "SELECT * FROM v6.arei_educ_staff_diversity_county_2024")

c_index_z <- st_read(con, query = "SELECT * FROM v6.arei_educ_index_2024")


####################### POPULATION DATA #####################################
#Join total population to index view, food access, and asthma data frames
pop <- st_read(con2, query = "SELECT geoid, dp05_0001e FROM demographics.acs_5yr_dp05_multigeo_2022")
...
####################### ADD STATE DATA #####################################
s_1 <- st_read(con, query = "SELECT * FROM v6.arei_educ_chronic_absenteeism_state_2024")
s_2 <- st_read(con, query = "SELECT * FROM v6.arei_educ_hs_grad_state_2024")
s_3 <- st_read(con, query = "SELECT * FROM v6.arei_educ_gr3_ela_scores_state_2024")
s_4 <- st_read(con, query = "SELECT * FROM v6.arei_educ_gr3_math_scores_state_2024")
s_5 <- st_read(con, query = "SELECT * FROM v6.arei_educ_suspension_state_2024")
s_6 <- st_read(con, query = "SELECT * FROM v6.arei_educ_ece_access_state_2024")
s_7 <- st_read(con, query = "SELECT * FROM v6.arei_educ_staff_diversity_state_2024")
...
<br><span style="color:green">Green</span> counties: Gains at Risk; 
...
* Central Valley counties are split between the Red and Yellow Quadrants, meaning they have low overall Education opportunities. Most counties also have higher disparity.
* Looking across Education indicators, Latinx, Black, and AIAN Californians fare worse than other groups.
...
c_1_chart <- c_1 %>% mutate(
    ...
    perf_z_cap = case_when(
      ...
      performance_z < -3.5 ~ -3.5,
      TRUE ~ performance_z)
    ) %>% filter(quadrant %in% c("green", "yellow", "orange", "red"))
 ...
 c_1_chart %>% 
  ...
  hc_caption(
    text = "Data source: CALIFORNIA DEPARTMENT OF EDUCATION (2020-21);<br> A chronically absent student is absent on 10 percent or more of school days per year.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
   hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
```
* San Mateo County is by far the most racially disparate on chronic absenteeism. NHPI, Black, and Latinx rates are three or more times higher than all other races.
* Del Norte County has by far the worst performance with more than half of students (53.3%) being chronically absent. All groups have higher rates, however, AIAN students are the most likely to be chronically absent (65.5%).
...
``` 
* Statewide, NHPI, Black, and AIAN students have the highest rates ranging from 22.4% to 26.9% respectively. These numbers increased from the 2018-19 school year, as may be expected as this data comes from the first year (2020-21) of the COVID-19 pandemic.
...
c_2_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA DEPARTMENT OF EDUCATION (2020-21)<br> Graduates are students who graduate with either a traditional high school diploma,<br>an adult education high school diploma, or have passed the California High School Proficiency Exam.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)

```
* Inyo, Mono, San Francisco, and Nevada Counties have among the highest racial disparities and lowest performance on high school graduation rates.
* In San Francisco County, less than half of Latinx and Black students graduate.
* Around three out of four White and AIAN students graduate in Inyo County, compared to only about one in three Latinx and Black students.
...
```
* Asian students, the group with the highest rate, have a graduation rate 21 percentage points higher than AIAN and Black students, the groups with the lowest rates. This gap has increased since the 2018-19 school year. This data comes from the first year (2020-21) of the COVID-19 pandemic when many students became disconnected from school.
...
c_3_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA ASSESSMENT OF STUDENT PERFORMANCE AND PROGRESS (2020-21);<br>Students scoring proficient or advanced on the third grade English Language Arts/Literacy (Smarter Balanced and California Alternate Assessment) test as a percentage of all third graders tested.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
```
* All Central Valley counties have lower performance. 
* Los Angeles jumped up (worsened) from a disparity rank of 41 to 8 compared to 2018-19 data, while performance rank dropped (worsened) from 15 to 30. It moved from the Green to Red Quadrant. This may be at least partially explained by the COVID-19 pandemic. It is important to note that only a small fraction of the usual number of students took the proficiency tests in the 2020-21 school year, so this chart represents a much smaller sample of students than other years.
...
``` 
* Statewide, two groups emerge. In the first group, fewer than one in three AIAN, Latinx, Black, and NHPI students scored proficient. While in the second group, more than 50% of Asian, Filipinx, Two or More Races, and White third graders scored proficient in English Language Arts. Note: A small percentage of the usual number of students took the proficiency test in the 2020-21 school year.
...
c_4_chart %>%
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA ASSESSMENT OF STUDENT PERFORMANCE AND PROGRESS (2020-21)<br>Students scoring proficient or advanced on the third grade Mathematics test (Smarter Balanced and California Alternate Assessment) as a percentage of all third graders tested.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
```
* Imperial, Riverside, and San Bernardino, representing half of the SoCal counties, are ranked 2nd, 3rd, and 4th most disparate in the state.
* Los Angeles jumped up (worsened) from a disparity rank of 25 to 8 compared to 2018-19 data, while performance rank dropped (worsened) from 14 to 30. It moved from the Orange to Red Quadrant. This may be at least partially explained by the COVID-19 pandemic. It is important to note that only a small fraction of the usual number of students took the proficiency tests in the 2020-21 school year, so this chart represents a much smaller sample of students than other years.
...
```
* Statewide, two groups emerge. In the first group, fewer than one in three AIAN, Black, Latinx, and NHPI students scored proficient. While in the second group, more than 50% of Asian, Filipinx, White, Two or More Races third graders scored proficient in math. Note: A small percentage of the usual number of students took the proficiency test in the 2020-21 school year.
...
c_5_chart  %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: California Department of Education (2020-21).<br>Students suspended include those who were suspended for any reason, including willful defiance.<br>This rate is calculated using cumulative enrolled students which consists of the total number of unduplicated primary and short-term enrollments within the academic year.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)

```
* Los Angeles is ranked 1 for performance with a suspension rate so low it rounds to zero. It dropped from the 23rd most disparate county to 48th during the same period (2018-19 compared to 2020-21). This change is likely due to decreased suspensions during the COVID-19 pandemic and distance learning in the 2020-21 school year.
* All Bay Area and SoCal counties, excluding Marin and San Diego, have lower disparity and higher performance putting them in the Green Quadrant. 
* Note that data for this time period may not reflect trends seen during in-person learning.
...
```
* Statewide, rates dropped across the board between 2018-19 and 2020-21 -- likely due to distance learning during the COVID-19 pandemic. Still, disparities remained with AIAN students being suspended twice (or more) as often as other students. Note that data for this time period may not reflect trends seen during in-person learning.
...
## ECE Access - NOT UPDATED {.tabset .tabset-fade}
...
c_6_chart %>% 
    hchart("scatter", 
    hcaes(x=disp_z_cap, y=perf_z_cap, size=county_total_und5, group=quadrant),
      tooltip = 
         list(pointFormat = 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA CHILD CARE RESOURCE & REFERRAL NETWORK (2019);<br>AMERICAN INSTITUTES FOR RESEARCH EARLY LEARNING NEEDS ASSESSMENT TOOL (2018);<br>Licensed childcare (CCCRRN) and Transitional Kindergarten (AIR) enrollment per 100 children aged 0-5 (AIR).<br>Accessible childcare refers to enrollment in one's neighborhood defined here as one's ZIP Code.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
```
* Marin, Colusa, Mono, and San Francisco Counties are the only counties where more than half of children ages 0-5 have access to a licensed ECE program. However, all four have higher than average racial disparities.
* All Central Valley counties have lower performance (access), with most having lower disparity as well.
...
``` 
* ECE access rates are low across the state and for all groups, even the best rate means only one in three children has access. Still Other Race, AIAN, Latinx, and Black children all have less ECE access than the average child in California.
...
c_7_chart %>% 
    hchart("scatter", 
    hcaes(x=disp_z_cap, y=perf_z_cap, size=total_enroll, group=quadrant),
      tooltip = 
         list(pointFormat = 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA DEPARTMENT OF EDUCATION (2018-19);<br>Ratio of staff of a race per 100 students of the same race.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* All Central Valley counties are in the Yellow (Lower disparity, Lower performance) and Red (Higher disparity, Lower performance) Quadrants.
...
``` 
* White students have by far the highest representation, with more than 13 White teachers and staff per 100 White students. Compare this to AIANs, the group with the next highest rate, at five per 100 students.
