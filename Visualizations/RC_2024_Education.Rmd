...
c_5_chart  %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: CALIFORNIA DEPARTMENT OF EDUCATION (2022-23).<br>Students suspended include those who were suspended for any reason, including willful defiance.<br>This rate is calculated using cumulative enrolled students which consists of the total number of unduplicated primary and short-term enrollments within the academic year.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)

```
* Inyo County is the most racially disparate for this measure, with suspension rates for AIAN students reaching 7.1 while the suspension rates for the lowest group is 0.

* Modoc county is the lowest performing for this measure, with suspension rates for NH-Black students reaching 38 and the lowest rate being 6.7.
