...
* On the Education Index, Central Valley counties are split between the Red and Yellow Quadrants, meaning they have low overall Education opportunities.(See scatterplot on Education Index)
...
* Additionally, NH-AIAN, NH-Black, Latinx, and NH-Pacific Islander students have lower graduation rates than the average rate for students across the state. (See State Barchart on Graduates as a Percentage of Four-Year Adjusted Cohort Students)
...
* NH- Flipinx, NH-Asian, NH-White, and NH- 3rd grade students of Two or More races experience higher average rates of proficiency in English and Math. (See State Barchart on 3rd Graders Scoring Proficient or Better in English Language Arts (%) and 3rd Graders Scoring Proficient or Better in Math (%))
...
* Central Valley counties have lower than average performance, with most counties also exhibiting higher levels of disparity. 
...
``` 
* Statewide, NHPI, Black, AIAN, and Latinx students have the highest rates for chronic absenteeism ranging from 28.9% to 36.6% respectively. 
...
```
* Most Central Valley counties exhibit below average performance for this measure.
...
* Modoc County is the lowest performing on this measure. Only 16.7% of Modoc's Latinx and 28.3% of its NH-White students score proficient or better in Math.  

* Larger population counties have higher levels of racial disparities among racial and ethnic groups for this indicator.
...
* Three counties (Modoc, Lake, and Del Norte) in the Northern California region are the lowest performing for this measure.
...
```
* Pacific Islander, Latinx,  AIAN, and Black students are suspended at above average rates. 

* NH_Black students are suspended at a rate 8.0 times higher than the lowest suspension rate. 
...
c_6_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: California Child Care Resource & Referral Network (2021); American Institutes for Research Early Learning Needs Assessment Tool (2020);<br>Licensed childcare (CCCRRN) and Transitional Kindergarten (AIR) enrollment per 100 children aged 0-5 (AIR).<br>Accessible childcare refers to enrollment in one's neighborhood defined here as one's ZIP Code.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
```
* Less than a quarter of San Bernardino County children ages 0-5 have access to a licensed ECE program, which is a 62.4 percentage point difference from the county with the most access. 
...
``` 
* ECE access rates are low across the state and for all groups, even the best rate means less than one in two children has access. Still AIAN, Latinx, and Black children all have less ECE access than the average child in California.
