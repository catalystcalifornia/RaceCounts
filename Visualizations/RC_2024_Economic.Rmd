...
library(tidyr)
library(stringr)
#library(rgdal) does not exist anymore
library(rpostgis)
...
#setwd("W:/Project/RACE COUNTS/2024_v6/Visualizations/") 
...
####################### ADDED V6 COUNTY DATA #####################################

c_1 <- st_read(con, query = "SELECT * FROM v6.arei_econ_living_wage_county_2024")  
c_2 <- st_read(con, query = "SELECT * FROM v6.arei_econ_employment_county_2024")
c_3 <- st_read(con, query = "SELECT * FROM v6.arei_econ_internet_county_2024")
c_4 <- st_read(con, query = "SELECT * FROM v6.arei_econ_per_capita_income_county_2024")
c_5 <- st_read(con, query = "SELECT * FROM v6.arei_econ_officials_county_2024")
c_6 <- st_read(con, query = "SELECT * FROM v6.arei_econ_real_cost_measure_county_2024")
c_7 <- st_read(con, query = "SELECT * FROM v6.arei_econ_connected_youth_county_2024")

c_index_z <- st_read(con, query = "SELECT * FROM v6.arei_econ_index_2024")
...
####################### ADD STATE DATA #####################################
s_1 <- st_read(con, query = "SELECT * FROM v6.arei_econ_living_wage_state_2024")  
s_2 <- st_read(con, query = "SELECT * FROM v6.arei_econ_employment_state_2024")
s_3 <- st_read(con, query = "SELECT * FROM v6.arei_econ_internet_state_2024")
s_4 <- st_read(con, query = "SELECT * FROM v6.arei_econ_per_capita_income_state_2024")
s_5 <- st_read(con, query = "SELECT * FROM v6.arei_econ_officials_state_2024")
s_6 <- st_read(con, query = "SELECT * FROM v6.arei_econ_real_cost_measure_state_2024")
s_7 <- st_read(con, query = "SELECT * FROM v6.arei_econ_connected_youth_state_2024")

# merge with urban type 
region_urban_type <- st_read(con, query = "SELECT geoid AS county_id, region, urban_type FROM v5.arei_multigeo_list") # Using v5 data for now, will update later with v6 data once available
...
* Rural counties and counties with large rural populations exhibit lower economic performance and greater disparity in economic opportunity and wages compared to their urban counterparts. (See scatterplot on Economic Opportunity Index and People ages 18-64 Earning A Living Wage (%))


* White and Asian residents across the counties have higher per capita income compared to other racial groups, who tend to earn below the average income rate for all groups. (See bar chart on Per Capita Income ($))

* NH_AIAN, NH-Black, and Latinx residents have incomes that fall below the cost-of-living on average across the counties. (See bar chart on Households Above Real Cost Measure ($))

* Pacific Islanders, Latinx, NH-AIAN, and NH-Black residents are far less likely to be employed as officials or managers. (See bar chart on People Ages 18-64 in labor Force who are Employed as Officials or Managers per 1k People)
...
*All counties that have rural populations larger than their urban populations have lower than average performance, with most having higher than average disparity as well. 

*All Bay Area counties have higher than average performance with low levels of disparity. However, Marin and San Francisco also have higher than average levels of disparity.
...
c_1_chart <- c_1 %>% mutate(
 ...
 c_1_chart %>% 
  ...
  hc_caption(
    text = "Data source: American Community Survey 5-Year PUMS Estimates (2018-2022)<br>Living Wage is defined as $15.50 or more per hour.<br> A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
   hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* All San Joaquin Valley counties have below average performance and all but two (Stanislaus and Kings) have above average levels of disparity on this measure. 

*All but two Bay Area counties (Marin and San Benito) have high performance and below average disparity measures. 
...
## Employment Status - UPDATED {.tabset .tabset-fade}
...
c_2_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: American Community Survey 5-Year Estimates, Table S2301 (2018-2022), TABLE S2301 (2016-20). <br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
c_3_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: American Community Survey 5-Year Estimates, Table S2802 (2018-2022)<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
c_4_chart %>%
  ...
  hc_caption(
    text = "Data source: American Community Survey 5-Year Estimates, Tables B19301B-I (2018-2022)<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* Counties with smaller populations tend to have lower per capita income performance than counties with larger populations. 
...
* Non-Hispanic White residents and Asian residents have above average rates for per capita income than the other groups. 
...
c_5_chart  %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: AMERICAN COMMUNITY SURVEY PUMS (2018-2022)<br>Officials and managers includes executive/senior level officials<br>and managers and first/mid-level officials and managers by residential address.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* San Luis Obispo has the worst racial disparity by far on this measure, in part due to lower rates for Latinx and Black residents. 
...
* Non-Latinx White workers are 2.4 times more likely to be employed as officials or managers than Latinx workers
...
c_6_chart %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: United Ways of California (2023)<br>The Real Cost Measure of poverty accounts for local costs of living, including the costs of housing, food,<br>health care, child care, transportation and other basic needs. It is a more accurate measure of financial security<br>than the federal poverty measure. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>%     #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* Marin County has the most racial disparity on this measure by far. 
...
c_7_chart  %>% 
  ...
  hc_yAxis(title = list(text = "Performance (Low -> High)"), max = 4, min = -4) %>% 
  hc_caption(
    text = "Data source: American Community Survey 5-Year PUMS Estimates (2018-2022)<br>Connected Youth are youth ages 16-24 who are in school and/or employed.<br>A Disparity Rank of 1 represents the most disparate. A Performance Rank of 1 represents the best outcomes.",
    align = "center"
     ) %>% #hc_size(height=400,width=700) %>% 
  hc_legend(element_blank) %>% hc_add_theme(rc_theme)
...
* Per capita income performance is close to average in all quadrants, except Red (Higher Disparity, Lower Performance). There is a lot of variation in performance (average Connected Youth Rate) among that group. 
...
* Black youth have a rate more than 13.4 percentage points lower than the group with the highest rate. 
