...
clean_data_z <- function(x, y, z) {
  ...
  x <- x %>% select(state_id, state_name, index_of_disparity) %>% 
    mutate(variable = y) %>%
    mutate(issue_area = z)
...
most_disp_issue <- c_index %>% group_by(issue_area) %>% summarize(avg_id = mean(index_of_disparity))

most_disp_indicator <- c_index %>% group_by(issue_area) %>% slice_max(index_of_disparity, n = 1) %>%
  arrange(desc(index_of_disparity))
