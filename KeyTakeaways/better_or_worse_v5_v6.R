...
clean_data_z <- function(x, y, z, updated="yes") {
...
c_1 <- clean_data_z(c_1, varname1, issue_area1, updated="no")
...
c_6 <- clean_data_z(c_6, varname6, issue_area2, updated="no")
c_7 <- clean_data_z(c_7, varname7, issue_area2, updated="no")
c_8 <- clean_data_z(c_8, varname8, issue_area2, updated="no")
c_9 <- clean_data_z(c_9, varname9, issue_area2)
c_10 <- clean_data_z(c_10, varname10, issue_area2)
c_11 <- clean_data_z(c_11, varname11, issue_area2, updated="no")
...
c_25 <- clean_data_z(c_25, varname25, issue_area4, updated="no")
...
c_32 <- clean_data_z(c_32, varname32, issue_area6, updated="no")
c_33 <- clean_data_z(c_33, varname33, issue_area6, updated="no")
c_34 <- clean_data_z(c_34, varname34, issue_area6, updated="no")
c_35 <- clean_data_z(c_35, varname35, issue_area6, updated="no")
...
c_41 <- clean_data_z(c_41, varname41, issue_area7, updated="no")
...
c_47 <- clean_data_z(c_47, varname47, issue_area7, updated="no")
...
indicators_better_outcomes_abs <- combined_data %>%
  filter(outcome_better=="better") %>%
  mutate(abs_pct_change = abs(rate_pct_chng))

indicators_worse_outcomes_abs <- combined_data %>%
  filter(outcome_better=="worse") %>%
  mutate(abs_pct_change = abs(rate_pct_chng))
...
id_change_sum_all <- combined_data %>% group_by(disparity_better) %>% summarize(all_id_pct_chng_sum = sum(id_pct_chng, na.rm=TRUE), 
                                                                            n=n())

id_change_sum_updated <- combined_data %>% 
  filter(updated_data=="yes") %>% 
  group_by(disparity_better) %>% summarize(updated_id_pct_chng_sum = sum(id_pct_chng, na.rm=TRUE), 
                                                                                n=n())

id_change_sum <- id_change_sum_all %>%
  left_join(id_change_sum_updated, by=c("disparity_better"))



issue_rate_change <- combined_data %>% group_by(outcome_better, issue_area) %>% summarize(sum_rate_pct_chng = sum(rate_pct_chng), abs_sum_rate_pct_chng = sum(abs(rate_pct_chng)), n=n()) %>%
  ungroup() %>%
  mutate(clean = ifelse(outcome_better=="worse", -1, 1),
         directional_chng = clean * abs_sum_rate_pct_chng)
