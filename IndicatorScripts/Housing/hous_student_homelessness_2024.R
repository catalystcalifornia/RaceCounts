...
df_subset <- df %>% filter(aggregatelevel %in% c("C", "T", "D") & charterschool == "All" & dass=="All" & reportingcategory %in% c("TA", "RB", "RI", "RA", "RF", "RH", "RP", "RT", "RW")) %>%
  ...
  select(cdscode, aggregatelevel, countycode, districtcode, countyname, districtname, reportingcategory, homelessstudentenrollment, cumulativeenrollment)
...
county_table_name <- "arei_hous_student_homelessness_county_2024"
state_table_name <- "arei_hous_student_homelessness_state_2024"
...
source <- "CDE 2022-23 https://dq.cde.ca.gov/dataquest/"
rc_schema <- 'v6'
