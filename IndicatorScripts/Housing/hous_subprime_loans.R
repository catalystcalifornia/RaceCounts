...
subprime_county_v3 <- dbGetQuery(con, "SELECT * FROM data.arei_subprime_county_2017")
subprime_city_v3 <- dbGetQuery(con, "SELECT * FROM data.arei_subprime_city")
...
# Filter for home purchases, first liean, one to four family home, owner occupancy

# Not Sure about Actions Taken filter. In the past, denied mortgage filters the all applications data to also include loan originated but this filter didn't make sense to me.
...
## function for filter calculations
...
## County ##
### not using cross-walk df because of potential duplicates
...
) %>%  filter(!is.na(geoid)) %>% select(geoid, geolevel, everything())
...
d <- rbind(df_county_state, df_city)
...
city_table2 <- d[d$geolevel == 'place', ] %>% select(-c(geolevel))
