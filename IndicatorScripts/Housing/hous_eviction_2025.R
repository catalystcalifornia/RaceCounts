...
ind_df_2010 <- df_wide %>% dplyr::rename(c("target_id" = "county_id", "target_name" = "county_name", "sub_id" = "fips")) # rename fields for WA fx
View(ind_df_2010)
...
cb_tract_2010_2020 <- fread("W:\\Data\\Geographies\\Relationships\\tract20_tract10\\cb_tract2020_tract2010_st06.txt", sep="|", colClasses = 'character', data.table = FALSE) %>%
  ...
  mutate(prc_overlap=AREALAND_PART/AREALAND_TRACT_10)
...
ind_2010_2020 <- ind_df_2010 %>%
  ...
  select(sub_id, sum_eviction, num_yrs, AREALAND_TRACT_10, GEOID_TRACT_20, AREALAND_TRACT_20, AREALAND_PART, prc_overlap)  %>%
...
ind_2020 <- ind_2010_2020 %>%
  ...
  mutate(avg_eviction = sum_eviction / num_yrs)
...
pop <- lapply(pop, function(x) x %>% rename(e = estimate, m = moe))
...
pop_wide_city <- dplyr::rename(pop_wide_city, sub_id = GEOID, target_id = place_geoid, target_name = place_name) # rename to generic column names for WA functions
...
city_wa <- city_wa %>% rename(target_name = place_name) %>% mutate(geolevel = 'city')                                  # change place_name to target_name, add geolevel
