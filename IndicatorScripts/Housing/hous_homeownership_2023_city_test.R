...
zcta_shp <- zctas(year = yr, cb = FALSE)  # non-cbf bc 2021 cbf zctas are not available yet, also not avail. by state
state_shp <- states(year = yr, cb = TRUE) %>% filter(, NAME == 'California') # cbf shape
zcta_shp <- st_intersection(zcta_shp, state_shp) %>%   # clip zctas to cbf state shape aka create our own CA-only cbf zcta shape
  dplyr::select(-c("STATEFP", "STATENS", "AFFGEOID", "GEOID", "STUSPS", "NAME", "LSAD","ALAND", "AWATER"))  # drop unneeded columns from state_shp

list_ca_zctas <- zcta_shp$zcta5ce20
