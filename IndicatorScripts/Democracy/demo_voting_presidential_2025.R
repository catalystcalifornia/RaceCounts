...
list.of.packages <- c("DBI", "tidyverse","RPostgres", "tidycensus")
...
#2024 metadata: https://www2.census.gov/programs-surveys/cps/techdocs/cpsnov24.pdf

#DATA CLEANING FUNCTION: add missing leading zeroes to geoids. add state fips (06) to CA county gtco's.
#see p31-33 of metadata for more: https://www2.census.gov/programs-surveys/cps/techdocs/cpsnov24.pdf
...
# Calc voting 
voted_county_2024 <- voted_by_county(df_2024)
names(voted_county_2024) <- ifelse(names(voted_county_2024) == "gtco", "geoid", paste0(names(voted_county_2024),"_24"))

voted_state_2024 <- voted_by_state(df_2024)
names(voted_state_2024) <- ifelse(names(voted_state_2024) == "gestfips", "geoid",  paste0(names(voted_state_2024),"_24"))

### merge voting
final_voted2024 <- rbind(voted_county_2024, voted_state_2024)

# Calc citizen voting age pop
eligible_county_2024<- voting_age_county(df_2024)
names(eligible_county_2024) <- ifelse(names(eligible_county_2024) == "gtco", "geoid",  paste0(names(eligible_county_2024),"_24"))

eligible_state_2024 <- voting_age_state(df_2024)
names(eligible_state_2024) <- ifelse(names(eligible_state_2024) == "gestfips", "geoid",  paste0(names(eligible_state_2024),"_24"))

### merge eligible voters
final_eligible2024 <- rbind(eligible_county_2024, eligible_state_2024)

# merge voting & citizen voting age pop
final_2024 <- merge(final_voted2024, final_eligible2024, by = "geoid")


# Merge all data years together
final <- full_join(final_2012, final_2016, by = "geoid")
final <- full_join(final, final_2020, by = "geoid")
final <- full_join(final, final_2024, by = "geoid")
...
names(final_2024) <- gsub(x = names(final_2024), pattern = "\\_24", replacement = "")  
final_2024$year <- "2024"

geoid_counts <- rbind(final_2012, final_2016)
geoid_counts <- rbind(geoid_counts, final_2020)
geoid_counts <- rbind(geoid_counts, final_2024)
...
final_df <-  final %>% mutate(
  num_total_va_pop = rowSums(select(.,num_total_va_pop_12, num_total_va_pop_16, num_total_va_pop_20, num_total_va_pop_24), na.rm = TRUE)/num_yrs,
  num_latino_va_pop = rowSums(select(.,num_latino_va_pop_12, num_latino_va_pop_16, num_latino_va_pop_20, num_latino_va_pop_24), na.rm = TRUE)/num_yrs,
  num_nh_white_va_pop = rowSums(select(.,num_nh_white_va_pop_12, num_nh_white_va_pop_16, num_nh_white_va_pop_20, num_nh_white_va_pop_24), na.rm = TRUE)/num_yrs,
  num_nh_black_va_pop = rowSums(select(.,num_nh_black_va_pop_12, num_nh_black_va_pop_16, num_nh_black_va_pop_20, num_nh_black_va_pop_24), na.rm = TRUE)/num_yrs,
  num_aian_va_pop =  rowSums(select(.,num_aian_va_pop_12, num_aian_va_pop_16, num_aian_va_pop_20,num_aian_va_pop_24), na.rm = TRUE)/num_yrs,
  num_nh_asian_va_pop = rowSums(select(.,num_nh_asian_va_pop_12, num_nh_asian_va_pop_16, num_nh_asian_va_pop_20, num_nh_asian_va_pop_24), na.rm = TRUE)/num_yrs,
  num_pacisl_va_pop = rowSums(select(.,num_pacisl_va_pop_12, num_pacisl_va_pop_16, num_pacisl_va_pop_20, num_pacisl_va_pop_24), na.rm = TRUE)/num_yrs,
  num_nh_twoormor_va_pop = rowSums(select(.,num_nh_twoormor_va_pop_12, num_nh_twoormor_va_pop_16, num_nh_twoormor_va_pop_20, num_nh_twoormor_va_pop_24), na.rm = TRUE)/num_yrs,
  
  count_total_voted = rowSums(select(.,count_total_voted_12, count_total_voted_16, count_total_voted_20, count_total_voted_24), na.rm = TRUE),
  count_latino_voted = rowSums(select(.,count_latino_voted_12, count_latino_voted_16, count_latino_voted_20, count_latino_voted_24), na.rm = TRUE),
  count_nh_white_voted = rowSums(select(.,count_nh_white_voted_12, count_nh_white_voted_16, count_nh_white_voted_20, count_nh_white_voted_24), na.rm = TRUE),
  count_nh_black_voted = rowSums(select(.,count_nh_black_voted_12, count_nh_black_voted_16, count_nh_black_voted_20, count_nh_black_voted_24), na.rm = TRUE), 
  count_aian_voted = rowSums(select(.,count_aian_voted_12, count_aian_voted_16, count_aian_voted_20, count_aian_voted_24), na.rm = TRUE), 
  count_nh_asian_voted = rowSums(select(.,count_nh_asian_voted_12, count_nh_asian_voted_16, count_nh_asian_voted_20, count_nh_asian_voted_24), na.rm = TRUE), 
  count_pacisl_voted = rowSums(select(.,count_pacisl_voted_12, count_pacisl_voted_16, count_pacisl_voted_20, count_pacisl_voted_24), na.rm = TRUE), 
  count_nh_twoormor_voted = rowSums(select(.,count_nh_twoormor_voted_12, count_nh_twoormor_voted_16, count_nh_twoormor_voted_20, count_nh_twoormor_voted_24), na.rm = TRUE), 
  
  num_total_voted = rowSums(select(.,num_total_voted_12, num_total_voted_16, num_total_voted_20, num_total_voted_24), na.rm = TRUE)/num_yrs,
  num_latino_voted = rowSums(select(.,num_latino_voted_12, num_latino_voted_16, num_latino_voted_20, num_latino_voted_24), na.rm = TRUE)/num_yrs,
  num_nh_white_voted = rowSums(select(.,num_nh_white_voted_12, num_nh_white_voted_16, num_nh_white_voted_20, num_nh_white_voted_24), na.rm = TRUE)/num_yrs,
  num_nh_black_voted = rowSums(select(.,num_nh_black_voted_12, num_nh_black_voted_16, num_nh_black_voted_20, num_nh_black_voted_24), na.rm = TRUE)/num_yrs,
  num_aian_voted =  rowSums(select(.,num_aian_voted_12, num_aian_voted_16, num_aian_voted_20, num_aian_voted_24), na.rm = TRUE)/num_yrs,
  num_nh_asian_voted = rowSums(select(.,num_nh_asian_voted_12, num_nh_asian_voted_16, num_nh_asian_voted_20, num_nh_asian_voted_24), na.rm = TRUE)/num_yrs,
  num_pacisl_voted = rowSums(select(.,num_pacisl_voted_12, num_pacisl_voted_16, num_pacisl_voted_20, num_pacisl_voted_24), na.rm = TRUE)/num_yrs,
  num_nh_twoormor_voted = rowSums(select(.,num_nh_twoormor_voted_12, num_nh_twoormor_voted_16, num_nh_twoormor_voted_20, num_nh_twoormor_voted_24), na.rm = TRUE)/num_yrs
) %>% 
...
county_table_name <- "arei_demo_voting_presidential_county_2024"
state_table_name <- "arei_demo_voting_presidential_state_2024"
...
source <- "CPS 2012, 2016, 2020, and 2024 average https://www.census.gov/topics/public-sector/voting/data.html"
