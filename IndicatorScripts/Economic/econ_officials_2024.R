...
ppl <- fread(paste0(root, "psam_p06.csv"), header = TRUE, data.table = FALSE, select = c(cols_, "AGEP", "ESR", "SCH", "SOCP", "PUMA10", "PUMA20", "ANC1P", "ANC2P", "HISP", "RAC1P", "RAC2P", "RAC3P", "RACAIAN", "RACPI", "RACNH"),
             colClasses = list(character = c("ESR", "SCH", "SOCP", "PUMA10", "PUMA20", "ANC1P", "ANC2P", "HISP", "RAC1P", "RAC2P", "RAC3P", "RACAIAN", "RACPI", "RACNH")))
...
# d_county_state <- screened %>% mutate(geolevel = ifelse(geoname=="California","State", "County")) #no need to rename while city is not updatable
d <- screened %>% mutate(geolevel = ifelse(geoname=="California","State", "County"))

# # ##### City Calculations ##################### not updateable so commented out for now
...
county_table <- county_table %>% dplyr::rename("county_name" = "geoname", "county_id" = "geoid")%>% 
  select(-c(#ends_with("rate_moe"), ends_with("rate_cv"), ends_with("pop"), 
            ...
            # white_diff, black_diff, asian_diff, other_diff,
            # white_disparity_z, black_disparity_z, asian_disparity_z, other_disparity_z, 
            # white_performance_z, black_performance_z, asian_performance_z, other_performance_z,
            geolevel))
View(county_table)
