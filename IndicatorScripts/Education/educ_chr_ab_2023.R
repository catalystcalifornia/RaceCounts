...
df_subset <- df %>% filter(aggregatelevel %in% c("C", "T", "D") & charterschool == "All" & 
...
districts <- st_read(con, query = "SELECT cdscode, ncesdist AS geoid FROM education.cde_public_schools_2021_22 WHERE ncesdist <> '' AND right(cdscode,7) = '0000000' AND statustype = 'Active'")
