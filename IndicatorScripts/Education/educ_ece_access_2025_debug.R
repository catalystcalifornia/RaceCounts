...
source("W:/RDA Team/R/Github/RDA Functions/AB/RDA-Functions/Cnty_St_Wt_Avg_Functions.R")     #set source for WA Functions script
...
# select census population under age 5 by race/eth (in this order): 
p12_race <- c("total", "aian", "pacisl", "latino", "nh_white", "nh_black", "nh_asian", "nh_other", "nh_twoormor")
...
# county_names <- function(var_list, yr, srvy){
  # targetgeo_names <- list(get_acs(geography = targetgeolevel, state = "CA", variables = vars_list_acs, year = yr, survey = srvy, cache_table = TRUE))
  targetgeo_names <- list(get_decennial(geography = targetgeolevel, state = "CA", variables = vars_list_p12, year = year, survey = survey, sumfile = "dhc", cache_table = TRUE))
...
#
#   return(targetgeo_names)
# }


# ### Get Sub Geolevel Pop (ACS) ####
# # update_detailed_table <- function(vars, yr, srvy){
#   if (subgeo == 'tract' & survey == "acs5") {
    # list(
    pop <- list(
      get_decennial(geography = "tract", state = "CA", variables = vars_list_p12, year = year, survey = survey, cache_table = TRUE) %>%
...
#     # list(
#     pop <- list(
#       get_decennial(geography = "zcta", variables = vars_list_p12, year = year, survey = survey) %>% 
