...
#   filepath = "https://www3.cde.ca.gov/demo-downloads/attendance/chronicabsenteeism23.txt"   # will need to update each year
...
#      table_name <- "cde_multigeo_chronicabs_2022_23"
...
df <- st_read(con, query = "SELECT * FROM education.cde_multigeo_chronicabs_2022_23") # comment out code to pull data and use this once rda_shared_data table is created
...
districts <- st_read(con, query = "SELECT cdscode, ncesdist AS geoid FROM education.cde_public_schools_2022_23 WHERE ncesdist <> '' AND right(cdscode,7) = '0000000' AND statustype = 'Active'")
