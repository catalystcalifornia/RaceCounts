## MOSAIC: Disaggregated Asian/NHPI Per Capita Income B19301 ###
...
############## UPDATE VARIABLES ##############
curr_yr = 2010      # 2010 bc this table wasn't included in 2021 SPT tables
rc_yr = '2025'      # you MUST UPDATE each year
rc_schema ="v7"     # you MUST UPDATE each year
schema = 'v7'
qa_filepath <- "W:\\Project\\RACE COUNTS\\2025_v7\\Economic\\QA_Per_Capita_Income - MOSAIC.docx"

cv_threshold = 30         
pop_threshold = NA       
asbest = 'max'            
issue = 'econ'                 # rc table issue prefix, eg: hous, econ, hben, etc.
ind_name = 'per_capita_income' # rc table indicator name, eg: per_capita_income, homeownership
table_code = 'b19301'          # Select relevant indicator table name
...
indicator <- paste0("Per Capita Income ($) ", str_to_title(race_name), " Detailed Groups ONLY")  # See most recent Indicator Methodology for indicator description
...
indicator <- paste0("Per Capita Income ($) ", toupper(race_name), " Detailed Groups ONLY")  # See most recent Indicator Methodology for indicator description
source <- paste0("ACS (", start_yr, "-", curr_yr,") 5-Year Estimates, SPT Table ", str_to_title(table_code), " https://data.census.gov/cedsci/ . QA doc: ", qa_filepath)   # See most recent Indicator Methodology for source info
