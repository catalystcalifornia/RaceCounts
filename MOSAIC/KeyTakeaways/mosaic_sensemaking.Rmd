...
tot_df <- dbGetQuery(con2, paste0("SELECT * FROM ", tot_schema, ".", tot_table_name)) %>% 
  select(total_rate)
...
b_chart <- hchart(
  df %>% arrange(-rate),   # add ifelse to sort asc/desc depending on asbest value
    type = "bar", 
    hcaes(y = round(rate, 1), x = subgroup_label), 
    color = mainblue,
    tooltip = list(
      pointFormat = "Rate: {point.rate:.1f}%<br>
      Raw: {point.raw:,.0f}"),
  ) %>% 
  ...
  hc_yAxis( title = list(text = ""),
            ...
            plotLines = list(
              list(
                value = tot_df$total_rate,   # or your total value
                color = "red",
                width = 2,
                dashStyle = "Solid",     # "Solid", "Dash", "Dot", "DashDot"
                zIndex = 5,              # draw on top of bars
                label = list(
                  text = paste0("Total Rate: ", round(tot_df$total_rate, 1), "%"),
                  align = "right",
